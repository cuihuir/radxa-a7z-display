...
PowerVR image. It combines the verified `display5`/`gpu8` desktop, packaged
...
| Kernel | `5.15.147-21.1-a733` · package `5.15.147-21.1+display5` |
...
Release assets include a ready-to-flash integrated XZ image, the hardened `display5` kernel, `gpu8`, recovery tooling, bilingual release notes, and SHA256 checksums.
...
| Windows-friendly image release | ✅ Working | `v0.3.3` integrates Debian 12 KDE, `display5`, `gpu8`, packaged XWayland/KWin acceleration, vendor `l1`, and the tested `a7z-recovery` early-fsck entry in one XZ image. |
| Small-screen native mode | ✅ Working | `FLY-HDMI-LCD7` runs at native `1024x600@60Hz` without stretching or cropping. |
| HDMI hotplug recovery | ✅ Working | `display5` keeps HDMI hardware changes synchronized with DRM atomic state; unplug/replug recovers automatically at SDDM without an xrandr or udev workaround. |
| Full display kernel package | ✅ Working | `5.15.147-21.1+display5` boots from `l0`, includes the A7Z DTB, and preserves the explicit vendor DTB and GPU blacklist on recovery entry `l1`. |
...
- `tools/package_a733_kernel_display.sh INPUT.deb A7Z.dtb Image KERNEL.config OUTPUT.deb` adds the A7Z initramfs size workaround and required board DTB, then produces the installable `+display5` kernel package.
...
- `tools/a733_desktop_soak_test.sh smoke|regression|soak` collects DRM, PowerVR, process, memory, thermal, frequency, and journal evidence during board stability tests.
...
- Includes `display5`, `gpu8`, packaged XWayland/KWin acceleration, vendor
...
sudo dd if=radxa-a733-debian12-kde-pvr-recovery-20260722.img of=/dev/<target-disk> bs=4M status=progress conv=fsync
sync
```
...
## Update To `display5` + `gpu8`

Boot the Debian 12 image first, then download these assets from
[`v0.3.3-a733-touchscreen-recovery`](https://github.com/cuihuir/radxa-a7z-display/releases/tag/v0.3.3-a733-touchscreen-recovery):

- `linux-image-5.15.147-21.1-a733_5.15.147-21.1+display5_arm64.deb`
...
```text
5.15.147-21.1-a733
Version: 5.15.147-21.1+display5
