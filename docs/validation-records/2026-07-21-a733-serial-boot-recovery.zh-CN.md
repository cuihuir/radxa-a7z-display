...
## HID 触摸回归复测 — 2026 年 7 月 22 日

首个发布候选虽然可以启动，但内核配置包含
`# CONFIG_HID_MULTITOUCH is not set`，导致 USB HID 触摸屏不可用。Release 随即
重新转为草稿。随后使用 `CONFIG_HID_MULTITOUCH=y` 重构内核，写入备用 SD 卡并在
目标 A7Z 上启动，确认触摸屏恢复正常后，才以
`5.15.147-21.1+display4` 内核包重新打包完整镜像。
