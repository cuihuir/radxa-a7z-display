...
面向 Radxa A7Z/Z7A 的 Debian 12 KDE 桌面镜像、HDMI 原生分辨率支持，以及可重复构建的 A733 显示内核项目。
...
Release 中包含可安装 `.deb`、带安全检查的部署脚本、BSP 源码补丁和 SHA256 校验文件。
...
第一次成功点亮桌面时，我非常兴奋，这也让项目不再只是一组研究笔记。GPU 加速、NPU、音频和 BSP 清理仍有大量工作，但现代 Debian 桌面的主路径已经得到证明。如果你也因为相同原因购买了 A733，或者同样认为这颗 SoC 的潜力没有被官方软件充分发挥，欢迎使用和参与这个项目。
...
| GPU 加速 | 未解决 | 当前 renderer 为 `llvmpipe`，尚未启用硬件 3D 加速。 |
| DRM render node | 未解决 | 首次验证只发现 `/dev/dri/card0`，没有独立 render node。 |
...
- 图形渲染仍是 `llvmpipe`，硬件 GPU 加速尚未启用。
- 首次验证只发现 `/dev/dri/card0`，没有独立 render node。
- HDMI 音频可见但尚未验证播放质量。
