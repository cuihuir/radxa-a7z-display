...
完整镜像。它整合已验证的 `display4`/`gpu8` 桌面、打包版 XWayland/KWin
...
发布资产包括可直接烧录的整合 XZ 镜像、强化后的 `display4` 内核、`gpu8`、recovery 工具、中英文发布说明和 SHA256 校验文件。
...
- `tools/package_a733_kernel_display.sh INPUT.deb A7Z.dtb Image KERNEL.config OUTPUT.deb`：加入 A7Z initramfs 大小修复和必需的板级 DTB，并生成 `+display4` 内核包。
